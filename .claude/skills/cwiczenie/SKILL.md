---
name: cwiczenie
description: 'Generuje 1-3 ćwiczenia w C# do samodzielnego rozwiązania przez ucznia, dopasowane do bieżącej lekcji i poziomu trudności. Pięć poziomów: rozgrzewka, główne, gwiazdka, od modułu 4 naprawa zepsutego programu z wiedza/przyklady/zepsute, a w lekcjach 5.3, 7.3, 9.4, 12.4 i 13.3 etap miniprojektu „Dziennik nauki". Użyj na końcu lekcji lub gdy uczeń prosi o "więcej zadań".'
---
...
Nazwa pliku: `NN-temat.cs`, gdzie `NN` to numer przykładu przypisany lekcji w `wiedza/INDEX.md` (np. lekcja 6.2 → `15-list.cs`, więc jej ćwiczenia to `15-list-a.cs`, `15-list-b.cs`, `15-list-c.cs`). Trzy poziomy z jednej lekcji → sufiksy `-a`, `-b`, `-c`.
...
- **Od lekcji 7.1 ćwiczenia ⭐ i ⚡ na metody mogą wymagać `Sprawdz`** — trzech wywołań `Sprawdz(opis, wynik, oczekiwane)` z kroku 3.G lekcji 7.1, w tym jednego przypadku brzegowego. To sprawdzenie w kodzie ucznia, nie xUnit; bez `[Fact]`, bez `dotnet test`.
